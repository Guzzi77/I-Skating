...
class FitManager {
	...
	var sportType = ActivityRecording.SPORT_INLINE_SKATING;	
    ...
    function stopRecording(save) {
        ...
        if (Toybox has :ActivityRecording ) {
        	if (hasSession()) {	                                    // stop the session
		       	if (save) {
    ...
    function createSession() {	
	    System.println("FitManager - createSession");
		if (sportType == ActivityRecording.SPORT_INLINE_SKATING) {
	    ...
	    if (sportType == ActivityRecording.SPORT_ICE_SKATING) {
		    return ActivityRecording.createSession({    
		        :name => "Ice Skating",
		...
		System.error("Failed to create session");
	    return null;
	}    
	...
	function recordAvgSessionData() {
		if (fieldCadenceAvg != null && fieldStrideLengthAvg != null && stepsAtStart != null && session != null && session.isRecording()) {
	    	...
	    	if (stepsDuringSession != null &&
				stepsDuringSession > 0 &&
				Activity.getActivityInfo().elapsedTime != null &&
